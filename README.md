...
```json
{
  ...
  "data": {
    ...
    "conditionLogic": "AND"  // 条件逻辑关系: "AND" 或 "OR"
  }
}
```
...
```json
{
  ...
  "data": {
    "label": "节点标签",
    "conditionMode": "check",
    "checkTarget": "dex",  // 检定目标属性
    "checkDice": "1d100"   // 检定使用的骰子
  }
}
```
...
```json
{
  ...
  "homepage": "项目链接",
  "diceRules": {
    "defaultDice": "1d100",       // 默认检定骰子
    "successMode": "lte",         // 成功判定模式: "lte"(≤) 或 "gte"(≥)
    "criticalSuccess": 5,          // 大成功阈值
    "criticalFailure": 96,         // 大失败阈值
    "criticalSuccessMode": "lte", // 大成功判定模式
    "criticalFailureMode": "gte"  // 大失败判定模式
  }
}
```
...
完整的配置文件示例请查看 `public/example_complex.json` 文件。
